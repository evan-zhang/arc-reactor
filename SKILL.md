...
7. **极简静默输出 (Silent Output Rule)**：用户不希望在聊天窗口看到长篇大论排版。你无论输出调研报告、实体卡片提取还是矛盾示警，**所有长文本必须仅物理落盘为文件**。聊天界面的回复必须极其精简：仅包含状态成功与否、2-3句话的核心高度总结，以及明确的本地结果文件路径，或直接将 Markdown 文件作为实体抛给用户。
