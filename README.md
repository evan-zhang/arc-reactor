...
### Case 3: 真正的检索 (智能 Query 工作流)
- **模拟输入**：“基于现有的私人知识库，帮我总结一下我们前几次收录情报中，对于 OpenClaw 和 LLM Wiki 的联系是怎么看的？”
- **预期考核**：
    1. Agent 绝不能立刻联网盲搜（搜索外网是爬虫的作用，不是 Query 的作用）。
    2. 它必须第一步先读大门黑板：`wiki/index.md`。
    3. 找到相关的 `[[双向链接]]` 词语后，再打开细分的卡片，融会贯通后作答。

### Case 4: 周末大扫除 (Lint 整理机制)
