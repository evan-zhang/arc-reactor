...
### Display Layer 字数弹性规则（RT-028 新增）

Display Layer 的字数限制按场景分层，不再是单一的 200 字硬限制：

| 场景 | 字数上限 | 说明 |
|------|---------|------|
| 简单问答/群聊回复 | ≤200 字 | 口语化，结论先行 |
| 调研报告类 | ≤600 字 | 可适当超出，结构：结论 → 关键发现 → 判断力 → 行动建议 |
| 竞品对比/深度分析 | ≤1000 字 | 背景 → 核心对比 → 各方优劣 → 建议 |

**核心原则**：
- 核心要点必须在前3行
- 后续内容按需展开
- 不用追求一次性把所有内容都塞进 Display Layer

### Archive Layer 规范

| 层级 | 上限 | 说明 |
|------|------|------|
| Display Layer（展示层） | 按场景分层 | 用户直接看到的回复 |
